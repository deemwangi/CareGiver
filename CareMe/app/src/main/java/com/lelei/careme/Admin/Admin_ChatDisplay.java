...
import com.lelei.careme.Helpers.Admin_UserAdapter;
import com.lelei.careme.Models.Chat;
import com.lelei.careme.Models.Patient_Details;
...
public class Admin_ChatDisplay extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference_doc = FirebaseDatabase.getInstance("https://careconnect-2b4ca-default-rtdb.firebaseio.com/").getReference("Users");
    ...
    private void readChats(String email) {
        if(usersList.size() == 0){
            mUsers.clear();
            Toast.makeText(Admin_ChatDisplay.this, "Care Giver has no Current Chats", Toast.LENGTH_SHORT).show();
            adminUserAdapter = new Admin_UserAdapter(Admin_ChatDisplay.this, mUsers, email);
            recyclerView.setAdapter(adminUserAdapter);
        }
        else {
            reference = FirebaseDatabase.getInstance("https://careconnect-2b4ca-default-rtdb.firebaseio.com/").getReference("Patient_Details");
